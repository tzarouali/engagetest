...
public abstract class BaseController extends Controller {
    ...
    protected <T> CompletionStage<T> withTransaction(Function<DSLContext, CompletionStage<T>> fun) {
        final Connection conn = safeConnection();

        return DSL.using(conn, SQLDialect.POSTGRES).transactionResultAsync(configuration ->
                fun.apply(DSL.using(configuration))
        ).thenComposeAsync(completionStage -> completionStage.thenApplyAsync(result -> {
            try {
                conn.commit();
                return result;
            } catch (SQLException e) {
